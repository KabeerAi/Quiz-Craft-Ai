...
genai.configure(api_key='AIzaSyCWpBU8kC1kf9lslOl421x298R-aNZ0ueg')
...
def generate_mcqs(image_path, num_mcqs):
    try:
        ...
        # Ensure we return only the requested number of MCQs
...
def result():
    try:
        ...
        correct_answers = sum(1 for i, mcq in enumerate(
            mcqs) if user_answers.get(str(i)) == mcq['correct_answer'])
